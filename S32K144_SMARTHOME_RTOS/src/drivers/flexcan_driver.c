...
void SHD_CAN0_Init(void) {
    ...
    CAN0->MCR = CAN_MCR_FRZ_MASK | CAN_MCR_HALT_MASK | CAN_MCR_IRMQ_MASK | CAN_MCR_MAXMB(31);
    while(!(CAN0->MCR & CAN_MCR_FRZACK_MASK));           // Freeze 진입 확인

    /* 비트 타이밍: 500 kbps @ 8 MHz */
...
void CAN0_ORed_0_15_MB_IRQHandler(void) {
    ...
    if (CAN0->IFLAG1 & (1 << 4)) {
        ...
        if (g_can_rx_callback) {
            g_can_rx_callback(id, rx_data, dlc);
        }
    }
}